...
figure; plot(w,abs(E.w).^2);title('spectrum before propagation')

%%%%%%%%spatial domain construction (transverse coordinates)%%%%%%%%%%

sigma = 100.0e-06; %spot size at z=0 in meters
z = (pi.*(sigma.^2)/lambda0)./4; %propagation length in meters (equal to half Rayleigh range in this example)
steps = 5;
dz = z/steps;
...
dn = n2*(abs(E.z_w(:,m)).^2);
L = exp(-1i*b(m)*(dn.^2)*dz./(n(m)));
E.z_w(:,m) = L.* E.z_w(:,m);
%end
E_t_z = ifftshift(ifft(fftshift(E.z_w),N,2));
end
end
E_w_z = fftshift(fft(ifftshift(E.z_w),N,2));
figure; plot(w,abs(E_w_z(1,:)).^2);title('spectrum after propagation')
figure;
plot(r,abs(E_t_z(:,512)).^2);title('spatial profile after propagation')
figure;
